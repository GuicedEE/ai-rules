...
### GuicedEE repository destinations

When authoring in the GuicedEE ai-rules repository, keep skills under `skills/`: use
`.system/` for GuicedEE, ActivityMaster, EntityAssist, and JWebMP ecosystem skills and
`.curated/` for general-purpose skills. Preserve an existing skill's location. Apply these
repository defaults only when the user has not requested another destination.
