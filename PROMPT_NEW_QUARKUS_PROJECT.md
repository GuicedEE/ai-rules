# ⚡ Starter Prompt — New Quarkus Project Using the Rules Repository

Ask your AI tool to copy this template and align with the Rules Repository. The AI will scaffold a modular, forward-only documentation structure (Pact → Rules → Guides → Implementation), set up topic indexes, and generate minimal Quarkus starter code where requested.
...
- Selected tech topics (tick):
  - Quarkus (Servlet-free, reactive-first stack):
    - [ ] Core project setup
...
- Quarkus policy: Target Quarkus 3.x on Java LTS using BOM-managed versions; see [core.rules.md](rules/generative/backend/quarkus/core.rules.md) for module layout and extension management. Use RESTEasy Reactive for HTTP APIs by default.
...
- Quarkus selections:
  - Use [Quarkus index](rules/generative/backend/quarkus/README.md) and component rules: [core](rules/generative/backend/quarkus/core.rules.md), [resteasy-reactive](rules/generative/backend/quarkus/resteasy-reactive.rules.md), [panache-persistence](rules/generative/backend/quarkus/panache-persistence.rules.md), [reactive-messaging](rules/generative/backend/quarkus/reactive-messaging.rules.md), [security](rules/generative/backend/quarkus/security.rules.md), [dev-services](rules/generative/backend/quarkus/dev-services.rules.md), [native-build](rules/generative/backend/quarkus/native-build.rules.md), [testing](rules/generative/backend/quarkus/testing.rules.md).
  - Align OpenAPI/Swagger with Quarkus SmallRye OpenAPI defaults; expose OpenAPI UI in non-prod only.
...
- Actions: initialize repo; add Rules Repository submodule; create modular docs; set up Quarkus skeletons; update links; CI/env alignment.
...
4. Create project RULES.md (outside submodule):
   ...
   - Link topic indexes:
     - Backend (Quarkus) — [README](rules/generative/backend/quarkus/README.md)
...
5. Create GUIDES.md with links to chosen modular entries (e.g., RESTEasy Reactive APIs; Panache persistence and migrations; reactive messaging; Dev Services; security flows; native build; testing). Use glossary-aligned terms consistently.
...
- [ ] Project RULES.md present, linking to enterprise RULES and topic indexes (Quarkus + structural)
