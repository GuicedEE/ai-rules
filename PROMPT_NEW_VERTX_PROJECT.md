# 🌀 Starter Prompt — New Vert.x Project Using the Rules Repository

Ask your AI tool to copy this template and align with the Rules Repository. The AI will scaffold a modular, forward-only documentation structure (Pact → Rules → Guides → Implementation), set up topic indexes, and generate minimal Vert.x starter code where requested.
...
- Vert.x selections:
  - Use [Vert.x index](rules/generative/backend/vertx/README.md) and guides: [oauth2](rules/generative/backend/vertx/vertx-5-oauth2-flow-guide.md), [postgres client](rules/generative/backend/vertx/vertx-5-postgres-client.md), [transactions](rules/generative/backend/vertx/vertx-5-transaction-handling.md), [service discovery](rules/generative/backend/vertx/vertx-service-discovery.md), [TCP EventBus bridge](rules/generative/backend/vertx/vertx-5-integration-tcp-evenetbus-bridge.md), [upgrade](rules/generative/backend/vertx/vertx-5-upgrade.md).
  - Prefer Vert.x-native features (Mutiny/Pg client, event bus) over servlet adapters.
...
- Actions: initialize repo; add Rules Repository submodule; create modular docs; set up Vert.x skeletons; update links; CI/env alignment.
...
4. Create project RULES.md (outside submodule):
   ...
   - Link topic indexes:
     - Backend (Vert.x 5) — [README](rules/generative/backend/vertx/README.md)
...
5. Create GUIDES.md with links to chosen modular entries (e.g., Vert.x HTTP + OAuth2; Postgres client and transactions; service discovery; EventBus bridge; upgrade guide). Use glossary-aligned terms consistently.
...
9. README updates
   - State adoption of Rules Repository, link submodule path, and link PACT/RULES/GUIDES/IMPLEMENTATION/GLOSSARY. Note Java LTS and Vert.x selections.
