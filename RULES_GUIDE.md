...
Important: Generated frontend assets (do not edit)
- Never add, use, or modify HTML (.html), TypeScript (.ts), or CSS (.css) files in any project referencing these rules. These files are generated by build/theme pipelines and any manual changes will be overwritten and rejected in review.
- To request or implement UI changes, update the appropriate generator or theme sources (e.g., Java component/view definitions, theme source modules like UWE-Web-Theme, or other designated template/generator modules).
...
## UWE Project Addendum (Reactive Migration Patterns)

This addendum summarizes UWE-specific rules and links to the authoritative living document under rules/README.md for contributors working on UWE-* modules (Staff-, Sessions-, Web-Server, etc.).

Authoritative reference: rules/README.md

Event pattern quick link:
- See rules/README.md → "Events: End-to-End Pattern (UI → Server → Action)" for how UI events map to Vert.x listeners and actions, with Delete Session Line as the canonical example.

State caching quick link:
- See rules/README.md → "State Caching: updateCache utilization (Actions vs Listeners)" for when and how to call service.updateCache on Farm, PackingSession, Staff, and Timesheets, and which DataFetch to publish afterwards.
- TL;DR: Only Actions mutate and call updateCache; Listeners orchestrate and may publish DataFetch for read-only refreshes. Always thread enterprise→system→identityToken to updateCache calls.

Barcode printing/scanning quick link:
- See rules/README.md → "Barcode Printing & Scanning (Pack Instructions)" for batch-per-day printing, scanning ingestion, and where barcode-specific results surface (scanned* fields) and the end-to-end chain.
- TL;DR: Barcodes are printed in unique daily batches per Pack Instruction; scans update scanned* aggregates via Sessions actions, then updateCache + PackingSessionDataFetch refreshes the UI.

Staff & Timesheets aggregates note:
- Results aggregate not only to Station/Line/Session/Farm but also to Staff and Timesheets. See rules/README.md → "Where results appear" for Staff/Timesheets coverage and publication channels (StaffDataFetch and staff timesheets listeners).

Actions vs Listeners (Execution Model):
- Actions execute synchronously. They perform all mutations to core state (Farm, PackingSession, Staff, Timesheets) and persist JSON/state snapshots to the database to preserve the exact sequence of updates.
- Listeners can execute asynchronously. They generally provide information, validate/normalize payloads, and schedule one or more Actions (and sometimes additional Listeners). Avoid direct core-state mutations in listeners; delegate to Actions.

Key rules (UWE-specific):
- Always resolve enterprise → system → identityToken with the current Mutiny.Session before any server/service call.
...
- Never pass null for system or identityToken to services like farmService.getFarm or packingSessionService.updateCache.
- Do not cache/hold system or token in fields on actions/listeners; resolve per request.
- Validate farm/packshed/session/station fetches and fail fast with clear error logs.
- Staff actions: follow CheckStaffChecksumAction/SetStaffChecksumAction patterns; publish staff updates to web after successful mutations.
- Sessions actions: thread system/token into updateCache/planNewSession/startNewSession/updateStation/timesheet updates.
- Web Server graders listeners: don’t modify DefaultGraderSelectListener; resolve chain, then group device messages by COM port with MultiTimedComPortSender.
- Prohibit legacy CallScoper and TaskGroupRequest; use COM-port grouped senders instead.

Web update DataFetch channels (UI refresh):
- FarmDataFetch → full FarmDTO
- PackingSessionDataFetch → full SessionDTO
- StaffDataFetch → full StaffDTO[]
- Staff Timesheets → existing per-session, per-staff flows

Payload style update (2025-09):
- Vert.x EventBus messages now deliver full typed objects by default. Prefer strongly typed DTO/message payloads in listeners and actions.
- Avoid Map<String, Object> parsing in new code; keep Map-based handlers only for legacy emitters that haven’t been upgraded yet.

Domain model quick reference (see README "UWE Domain Model & Web Payloads"):
- Farm → Packing Sheds (1–4 Servers each) → Graders (2–255, up to 253 per server)
- Pack Instructions: farm-wide; up to 3 materials; Material Image default index 0 (none)
- Sessions: Session → Lines → Stations; Station has PackInstructionGroup (1–5 instructions)
- Staff assignments per Station: 0–5, with Unassigned staff id -10111 when none
- Aggregations propagate Station → Line → Session → Farm and per Staff/Timesheets; publish appropriate DataFetch after mutations

See rules/README.md for full details, examples, and before/after snippets.


## Web-Assist UI Conventions (2025-09-21)
- Planning/Status panels should not be empty when no line is selected. Provide a planning callout and a read-only overview of lines.
- Use WaCallout components as section headings for important context (e.g., “Planning phase”, “Bonus Configuration”).
- Target Reach (EKW) is presented as a read-only field in planning. Allow editing of units and rates, not the base EKW.
- Prefer read-only displays for computed/base values; reserve inputs for user-adjustable parameters.
